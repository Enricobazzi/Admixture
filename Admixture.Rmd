...
We will divide these VCFs by the populations we are interested in (vl=vladivostok, pv=pais vasco and cr=carpathians from Lynx lynx; sm=sierra morena from Lynx pardinus). Once we have these VCFs, we will intersect them in order to keep only the common positions between the three. We can then go to the next step which is PHASING.

## Get Hybrid individual (pv = pais vasco) and its closest population's (cr = carpathians) VCF to phase
...
cd $Admx_PATH
rm pv.cr_list_to_remove.txt

# define which individuals to extract
$BCF query -l $Data_PATH/ll_perspecies_filtered2_1mbpnoxyscaffolds.lr_ann.vcf | grep 'pv\|cr' > $Admx_PATH/pv.cr_list_to_remove.txt
cat $Admx_PATH/pv.cr_list_to_remove.txt

# GATK
screen -S GATKpartitionPV.log
script GATKpartitionPV.log
...
java -XX:MaxMetaspaceSize=1g -XX:+UseG1GC -XX:+UseStringDeduplication -Xms16g -Xmx32g -jar $GATK \
    ...
    -o $Data_PATH/ll_pv.cr_perpop_filtered2_1mbpnoxyscaffolds.lr_ann.vcf \
    --sample_file $Admx_PATH/pv.cr_list_to_remove.txt

rm $Admx_PATH/pv.cr_list_to_remove.txt

grep -v "#" $Data_PATH/ll_pv.cr_perpop_filtered2_1mbpnoxyscaffolds.lr_ann.vcf | wc -l #2786031

```

## Get Vladivostok VCF to phase - (not already generated by Dani)
...
cd $Admx_PATH
rm vl_list_to_remove.txt

# define which individuals to extract
$BCF query -l $Data_PATH/ll_perspecies_filtered2_1mbpnoxyscaffolds.lr_ann.vcf | grep 'vl' > $Admx_PATH/vl_list_to_remove.txt
cat $Admx_PATH/vl_list_to_remove.txt

# GATK
screen -S GATKpartitionVL.log
script GATKpartitionVL.log

java -XX:MaxMetaspaceSize=1g -XX:+UseG1GC -XX:+UseStringDeduplication -Xms16g -Xmx32g -jar $GATK \
    -T SelectVariants \
    -R $REF \
    -V $Data_PATH/ll_perspecies_filtered2_1mbpnoxyscaffolds.lr_ann.vcf \
    -o $Data_PATH/ll_vl_perpop_filtered2_1mbpnoxyscaffolds.lr_ann.vcf \
    --sample_file $Admx_PATH/vl_list_to_remove.txt

rm $Admx_PATH/vl_list_to_remove.txt

grep -v "#" $Data_PATH/ll_vl_perpop_filtered2_1mbpnoxyscaffolds.lr_ann.vcf | wc -l #2786031
...
$BCF index ll_vl_perpop_filtered2_1mbpnoxyscaffolds.lr_ann.vcf.gz
$BCF index ll_pv.cr_perpop_filtered2_1mbpnoxyscaffolds.lr_ann.vcf.gz

## intersect with BCFtools results in separate folder
$BCF isec -p $Data_PATH/intersect -n=3 lp_sm_perpop_filtered2_1mbpnoxyscaffolds.lr_ann.vcf.gz ll_vl_perpop_filtered2_1mbpnoxyscaffolds.lr_ann.vcf.gz ll_pv.cr_perpop_filtered2_1mbpnoxyscaffolds.lr_ann.vcf.gz
...
# /home/ebazzicalupo/Data/intersect/0001.vcf      for stripped    ll_vl_perpop_filtered2_1mbpnoxyscaffolds.lr_ann.vcf.gz
# /home/ebazzicalupo/Data/intersect/0002.vcf      for stripped    ll_pv.cr_perpop_filtered2_1mbpnoxyscaffolds.lr_ann.vcf.gz
...
mv 0001.vcf $Data_PATH/ll_vl_perpop_filtered2_1mbpnoxyscaffolds_intersect.lr_ann.vcf
mv 0002.vcf $Data_PATH/ll_pv.cr_perpop_filtered2_1mbpnoxyscaffolds_intersect.lr_ann.vcf

# check number of SNPs
grep -v "#" $Data_PATH/lp_sm_perpop_filtered2_1mbpnoxyscaffolds_intersect.lr_ann.vcf | wc -l #2752014
grep -v "#" $Data_PATH/ll_vl_perpop_filtered2_1mbpnoxyscaffolds_intersect.lr_ann.vcf | wc -l #2752014
grep -v "#" $Data_PATH/ll_pv.cr_perpop_filtered2_1mbpnoxyscaffolds_intersect.lr_ann.vcf | wc -l #2752014

## Intersect with BEDtools - Used BEDtools in the end
bedtools intersect -header -a $Data_PATH/lp_sm_perpop_filtered2_1mbpnoxyscaffolds.lr_ann.vcf -b $Data_PATH/ll_vl_perpop_filtered2_1mbpnoxyscaffolds.lr_ann.vcf > $Data_PATH/lp_sm_perpop_filtered2_1mbpnoxyscaffolds_intersected.lr_ann.vcf

# vl population
bedtools intersect -header -a $Data_PATH/ll_vl_perpop_filtered2_1mbpnoxyscaffolds.lr_ann.vcf -b $Data_PATH/lp_sm_perpop_filtered2_1mbpnoxyscaffolds.lr_ann.vcf > $Data_PATH/ll_vl_perpop_filtered2_1mbpnoxyscaffolds_intersected.lr_ann.vcf

# pv population
bedtools intersect -header -a $Data_PATH/ll_pv.cr_perpop_filtered2_1mbpnoxyscaffolds.lr_ann.vcf -b $Data_PATH/lp_sm_perpop_filtered2_1mbpnoxyscaffolds.lr_ann.vcf > $Data_PATH/ll_pv.cr_perpop_filtered2_1mbpnoxyscaffolds_intersected.lr_ann.vcf

# check number of SNPs
grep -v "#" $Data_PATH/lp_sm_perpop_filtered2_1mbpnoxyscaffolds_intersected.lr_ann.vcf | wc -l #2752014
grep -v "#" $Data_PATH/ll_vl_perpop_filtered2_1mbpnoxyscaffolds_intersected.lr_ann.vcf | wc -l #2752014
grep -v "#" $Data_PATH/ll_pv.cr_perpop_filtered2_1mbpnoxyscaffolds_intersected.lr_ann.vcf | wc -l #2752014
